- [SAA資格取得](#saa資格取得)
  - [1. 勉強方法](#1-勉強方法)
    - [1-1. 過去問の解き方・勉強方法](#1-1-過去問の解き方勉強方法)
  - [2. Storageサービス](#2-storageサービス)
    - [2-1. オブジェクトストレージ（高耐久・大容量）](#2-1-オブジェクトストレージ高耐久大容量)
      - [2-1-1.  用途](#2-1-1--用途)
      - [2-1-2. 構成要素](#2-1-2-構成要素)
      - [2-1-3. ストレージクラス](#2-1-3-ストレージクラス)
      - [2-1-4. ライフサイクル管理](#2-1-4-ライフサイクル管理)
      - [2-1-5. Glacierとは？](#2-1-5-glacierとは)
      - [2-1-6. S3とは？](#2-1-6-s3とは)
      - [2-1-7. S3のその他の機能](#2-1-7-s3のその他の機能)
    - [2-2. Computing Serviceにおけるストレージ](#2-2-computing-serviceにおけるストレージ)
      - [2-2-1. その他関連システム](#2-2-1-その他関連システム)
    - [2-3. ファイルストレージ（スケーラブルな共有ストレージ）](#2-3-ファイルストレージスケーラブルな共有ストレージ)
      - [2-3-1. Elastic File System（EFS）](#2-3-1-elastic-file-systemefs)
      - [2-3-2. FSx for Windows](#2-3-2-fsx-for-windows)
      - [2-3-3. FSx for Lustre](#2-3-3-fsx-for-lustre)
    - [2-4. ブロックストレージ（永続可能なブロックストレージ）](#2-4-ブロックストレージ永続可能なブロックストレージ)
    - [2-5. データ転送](#2-5-データ転送)
      - [2-5-1. Storage Gateway](#2-5-1-storage-gateway)
      - [2-5-2. Snow Family](#2-5-2-snow-family)
  - [3. EC2（Elastic Compute Cloud）](#3-ec2elastic-compute-cloud)
    - [3-1. 特徴](#3-1-特徴)
    - [3-2. 課金](#3-2-課金)
    - [3-3. 用語](#3-3-用語)
    - [3-4. インスタンスタイプ](#3-4-インスタンスタイプ)
    - [3-5. その他、EC2の機能、注意事項など](#3-5-その他ec2の機能注意事項など)
      - [3-5-1. プレイスメントグループ](#3-5-1-プレイスメントグループ)
      - [3-5-2. Elastic Network Interface（ENI）](#3-5-2-elastic-network-interfaceeni)
      - [3-5-3. ブートストラップ（ユーザデータの利用）](#3-5-3-ブートストラップユーザデータの利用)
      - [3-5-4. ゴールデンイメージ](#3-5-4-ゴールデンイメージ)
      - [3-5-5. RAID](#3-5-5-raid)
      - [3-5-6. EC2からEBSをデタッチする際の注意点](#3-5-6-ec2からebsをデタッチする際の注意点)
  - [4. Scaling](#4-scaling)
    - [4-1. スケーラビリティ](#4-1-スケーラビリティ)
    - [4-2. Auto Scaling](#4-2-auto-scaling)
      - [4-2-1. ヘルスチェック](#4-2-1-ヘルスチェック)
      - [4-2-2. クールダウン期間](#4-2-2-クールダウン期間)
      - [4-2-3. Desired capacity](#4-2-3-desired-capacity)
      - [4-2-4. スケーリングの種類](#4-2-4-スケーリングの種類)
      - [4-2-5. ターミネーションポリシー](#4-2-5-ターミネーションポリシー)
    - [4-3. Elastic Load Balancer](#4-3-elastic-load-balancer)
      - [4-3-1. ELBの種類](#4-3-1-elbの種類)
      - [4-3-2. スティッキーセッション](#4-3-2-スティッキーセッション)
      - [4-3-3. Connection Draining](#4-3-3-connection-draining)
      - [4-3-4. プロキシプロトコル](#4-3-4-プロキシプロトコル)
      - [4-3-5. SSL Termination](#4-3-5-ssl-termination)
      - [4-3-6. クロスゾーンロードバランシング](#4-3-6-クロスゾーンロードバランシング)
      - [4-3-7. 注意点](#4-3-7-注意点)
  - [5. コンテナーとは](#5-コンテナーとは)
    - [5-1. コンテナ](#5-1-コンテナ)
    - [5-2. コンテナーサービス](#5-2-コンテナーサービス)
      - [5-2-1. 説明](#5-2-1-説明)
      - [5-2-2. ホスティング先](#5-2-2-ホスティング先)
      - [5-2-3. 構成](#5-2-3-構成)
      - [5-2-4. その他](#5-2-4-その他)
  - [6. ネットワークサービス](#6-ネットワークサービス)
    - [6-1. 用語](#6-1-用語)
    - [6-2. VPC](#6-2-vpc)
    - [6-3. サブネット](#6-3-サブネット)
    - [6-4. ルートテーブル](#6-4-ルートテーブル)
    - [6-5. セキュリティテーブル vs ネットワークACL](#6-5-セキュリティテーブル-vs-ネットワークacl)
    - [6-6. ゲートウェイ](#6-6-ゲートウェイ)
    - [6-7. VPN](#6-7-vpn)
    - [6-8. VPCエンドポイント](#6-8-vpcエンドポイント)
    - [6-9. VPCピアリング](#6-9-vpcピアリング)
    - [6-10. VPCフローログ](#6-10-vpcフローログ)
    - [6-11. Route 53](#6-11-route-53)
      - [6-11-1. DNS機能](#6-11-1-dns機能)
      - [6-11-2. 用語](#6-11-2-用語)
      - [6-11-3. ルーティング](#6-11-3-ルーティング)
      - [6-11-4. その他](#6-11-4-その他)
    - [6-12. CouldFront](#6-12-couldfront)
      - [6-12-1. 用途](#6-12-1-用途)
      - [6-12-2. 設定](#6-12-2-設定)
      - [6-12-3. その他](#6-12-3-その他)
  - [7. データベースサービス](#7-データベースサービス)
    - [7-1. データベースの種類](#7-1-データベースの種類)
    - [7-2. RDS](#7-2-rds)
      - [7-2-1. ストレージタイプ](#7-2-1-ストレージタイプ)
      - [7-2-2. 特徴](#7-2-2-特徴)
      - [7-2-3. セキュリティ](#7-2-3-セキュリティ)
      - [7-2-4. その他設定](#7-2-4-その他設定)
    - [7-3. Redshift](#7-3-redshift)
      - [7-3-1. 特徴](#7-3-1-特徴)
      - [7-3-2. 機能](#7-3-2-機能)
    - [7-4. Aurora](#7-4-aurora)
      - [7-4-1. 特徴](#7-4-1-特徴)
      - [7-4-2. 構成](#7-4-2-構成)
      - [7-4-3. その他](#7-4-3-その他)
    - [7-5. DynamoDB](#7-5-dynamodb)
      - [7-5-1. 特徴](#7-5-1-特徴)
      - [7-5-2. パフォーマンス](#7-5-2-パフォーマンス)
      - [7-5-3. その他の機能](#7-5-3-その他の機能)
    - [7-6. Elastic Cache](#7-6-elastic-cache)
      - [7-6-1. 特徴](#7-6-1-特徴)
      - [7-6-2. 用途](#7-6-2-用途)
      - [7-6-3. 種類](#7-6-3-種類)
  - [8. 監視・セキュリティ](#8-監視セキュリティ)
    - [8-1. CloudWatch 運用監視サービス](#8-1-cloudwatch-運用監視サービス)
      - [8-1-1. CloudWatch](#8-1-1-cloudwatch)
      - [8-1-2. CloudWatch Logs](#8-1-2-cloudwatch-logs)
      - [8-1-3. CloudWatch Events](#8-1-3-cloudwatch-events)
    - [8-2. CloudTrail 操作ログを取得](#8-2-cloudtrail-操作ログを取得)
      - [8-2-1. 特徴](#8-2-1-特徴)
      - [8-2-2. 機能](#8-2-2-機能)
    - [8-3. その他のログ管理](#8-3-その他のログ管理)
      - [8-3-1. VPCフローログ](#8-3-1-vpcフローログ)
      - [8-3-2. AWS Config](#8-3-2-aws-config)
      - [8-3-3. AWS Systems Manager](#8-3-3-aws-systems-manager)
      - [8-3-4. AWS Trusted Advisor](#8-3-4-aws-trusted-advisor)
    - [8-4. セキュリティ関連サービス](#8-4-セキュリティ関連サービス)
      - [8-4-1. CloudHSM](#8-4-1-cloudhsm)
      - [8-4-2. AWS Key Management Service（KMS）](#8-4-2-aws-key-management-servicekms)
      - [8-4-3. AWS Certificate Manager(ACM)](#8-4-3-aws-certificate-manageracm)
      - [8-4-4. AWS Shield](#8-4-4-aws-shield)
      - [8-4-5. AWS Firewall Manager](#8-4-5-aws-firewall-manager)
      - [8-4-6. AWS Security Token Service(AWS STS)](#8-4-6-aws-security-token-serviceaws-sts)
      - [8-4-7. Amazon Inspector](#8-4-7-amazon-inspector)
    - [8-5. IDフェデレーション](#8-5-idフェデレーション)
      - [8-5-1. 発展編](#8-5-1-発展編)
      - [8-5-2. Single Sign-On](#8-5-2-single-sign-on)
    - [8-6. Active Directory](#8-6-active-directory)
  - [9. デプロイと自動化](#9-デプロイと自動化)
    - [9-1. Elastic Beanstalk](#9-1-elastic-beanstalk)
      - [9-1-1. インフラの自動構築](#9-1-1-インフラの自動構築)
      - [9-1-2. デプロイ方式](#9-1-2-デプロイ方式)
    - [9-2. OpsWorks](#9-2-opsworks)
      - [9-2-1. 構成管理サービス](#9-2-1-構成管理サービス)
      - [9-2-2. 用語](#9-2-2-用語)
      - [9-2-3. Chefの作動方式](#9-2-3-chefの作動方式)
    - [9-3. CloudFormation](#9-3-cloudformation)
      - [9-3-1. アプリの自動構築](#9-3-1-アプリの自動構築)
      - [9-3-2. テンプレートの記述例](#9-3-2-テンプレートの記述例)
    - [9-4. その他デプロイツール](#9-4-その他デプロイツール)
  - [10. アプリケーションサービス](#10-アプリケーションサービス)
    - [10-1. SQS（Simple Queue Service）](#10-1-sqssimple-queue-service)
      - [10-1-1. キューイングサービス](#10-1-1-キューイングサービス)
      - [10-1-2. 順序管理](#10-1-2-順序管理)
      - [10-1-3. メッセージの取得方法](#10-1-3-メッセージの取得方法)
      - [10-1-4. その他のキューの管理](#10-1-4-その他のキューの管理)
    - [10-2. ワークフロー](#10-2-ワークフロー)
    - [10-3. SNS（Simple Notification Service）](#10-3-snssimple-notification-service)
      - [10-3-1. 通知サービス](#10-3-1-通知サービス)
    - [10-4. SES（Simple Email Service）](#10-4-sessimple-email-service)
      - [10-4-1. メール配信サービス](#10-4-1-メール配信サービス)
  - [11. その他サービス](#11-その他サービス)
    - [11-1. Lambda](#11-1-lambda)
      - [11-1-1. 特徴](#11-1-1-特徴)
    - [11-2. Kinesis](#11-2-kinesis)
    - [11-3. ビッグデータ・IoT](#11-3-ビッグデータiot)
    - [11-4. 移行系サービス](#11-4-移行系サービス)
    - [11-5. コスト管理サービス](#11-5-コスト管理サービス)
...
MFA、SSOなど多様化するアクセス方法において、IDを統合的に連携するサービス

![](2023-03-15-12-16-01.png)

![](2023-03-15-12-15-49.png)

#### 8-5-2. Single Sign-On

- Amazon Single Sign-On（SSO）
  - AWS Organizationsにあるすべてのアカウントに対するアクセスとユーザアクセス許可を簡単に一元管理
  - Salesforce、BoxおよびMicrosoft365などの多数のビジネスアプリケーションとの統合機能
  - AWS SSOのIDストアでユーザIDを作成して管理

- Amazon Cognito
  - アプリケーションの認証機能を実装
  - OpenID Connect, OAuth 2.0, SAML 2.0などの一般的なID管理規格
...
- Chef Clientローカル方式
  - サーバ毎にRecipeを持ち、自サーバに適用する
- Chef Server/Client方式
  - マスターサーバを用意し、サーバ舞の適用状況を管理する
...
テンプレートを用いてアプリを構築する
複数アプリを作成するときなど、マネジメントコンソール経由で手動作成する手間を省く

- テンプレート
  - アプリの仕様が記載されたファイル（JSON, YAML）
- スタック
  - CloudFormationで構築されたAWSリソース

![](2023-03-15-13-29-34.png)

#### 9-3-2. テンプレートの記述例

![](2023-03-15-13-30-24.png)

### 9-4. その他デプロイツール

- AWS SAM（Serverless Application Model）：サム
...
- 2004年創業当初から運用開始
- システム間を非同期で連携し、疎結合とする。
  - （疎結合とは 疎結合（loose coupling）とは、システムが構成要素の組みあわせで作られており、それぞれの構成要素同士の依存関係が高くなく、互いに独立性が高い（密ではなく疎である）状態のこと）
- キューとメッセージの管理
- キューの使用例
  - テキストファイルをWebアプリにアップロードし、別アプリで入稿作業を行う時の順序管理
  - DBへ大量の書き込みをしたとき、オーバーフローを防ぐため、一旦SQSにバッファ（順番待ち）した後、DBへ処理を行う

![](2023-03-15-13-37-27.png)
...
- Standardキュー
  - 従来のメッセージ配信順序
- FIFOキュー
  - 先入れ先出しでメッセージ配信、二重配信の排除も保証（2016年〜）

![](2023-03-15-13-39-34.png)

#### 10-1-3. メッセージの取得方法

- ショートポーリング
  - リクエストを受け取るとメッセージの有無に関わらずレスポンスを返す（デフォルト）
- ロングポーリング
  - メッセージがない場合はタイムアウト時にレスポンスを返す

#### 10-1-4. その他のキューの管理

メッセージの生合成を保つため、下記のような機能が備えられている

- 可視性タイムアウト（Visibility Timeout）
  - ユーザがメッセージを受信して処理した時、はじめてキューからメッセージを消す
  - 30秒〜12時間で設定可能
  - 用途例:
    - 重複してメッセージを取得するのを防ぐ
    - 接続不具合などでメッセージを重複受信するのを防ぐ

![](2023-03-15-13-47-06.png)

- 遅延キュー（Delay Queue）
  - キュー内の指定したメッセージを一定時間見えなくするようにタイマーを設ける
  - 用途例:
    - ユーザ側のアプリがメッセージ処理までに時間を要す場合、時間生合成を保つ

![](2023-03-15-13-47-18.png)

- デッドレターキュー（Dead Letter Queue）
  - 処理に失敗したメッセージを別のキュー（ソースキュー）に移動
  - 用途例:
    - 問題のあるメッセージを分離して、原因を調査する

### 10-2. ワークフロー

SWFよりStep Functionの方が後発。
Step Functionsの方が実装が簡単。

- Amazon Simple Workflow（SWF）
  - 分散したアプリケーションコンポーネント間の作業調整
  - 詳細な管理ができる分、開発が複雑

![](2023-03-15-13-50-43.png)

- AWS Step Functions
  - 分散型アプリケーションや、マイクロサービスの構成要素を、ビジュアルなワークフローを用いて簡単に調整
  - ステートは、作業の実行、選択、パラメータの受け渡し、並列実行の開始、タイムアウトの管理、成功or失敗によるワークフローの終了などを行う
  - JSON形式でステップを管理

![](2023-03-15-13-49-49.png)

- Amazon SQS
  - メッセージ指向なので、タスクやフローの全体間の追跡は出来ない


### 10-3. SNS（Simple Notification Service）

#### 10-3-1. 通知サービス

- プッシュ型の通知サービス
  - サーバからユーザへ通知すること <=> プル型
- SMS, email, HTTP/HTTPS, SQS, モバイル端末へのプッシュ通知, Lambdaへ通知
- プロトコルの変換はSNS側で対応
- 用途例:
  - CloudWatchでアプリのメトリクスが閾値を超えたときにSNSによりプッシュ通知

![](2023-03-15-13-56-01.png)

### 10-4. SES（Simple Email Service）

#### 10-4-1. メール配信サービス

- SMTPプロトコルを使ってEメールを送信する
- Eメールを受信し、S3に保存
- セキュリティや証跡機能を持つ
  - ウィルスやマルウェアの検出
  - 送信成功/拒否数の記録
...
CloudWatchなどと連携し、以上検出したらメールで通知させる、という用途もある。

## 11. その他サービス

### 11-1. Lambda

Lambdaは、他のサービスと連携して実力を発揮するサービス

#### 11-1-1. 特徴

- コスト
  - 関数の実行数に応じて課金
    - Lambda関数が無限に実行されないようタイムアウト設定がある
      - デフォルトは3秒、リクエストあたりの最大実行時間は900秒（=15分）

- 使用言語
  - Java, Node.js, C#, Python, Go, Powershell, Rubyのプログラム言語に対応

- サイズ
  - Lambda関数は最大512MBまでのデータ容量（/tmpのディレクトリ）を扱う
  - 同時実行数は1,000
  - 関数とレイヤーストレージは75GB
...
- サーバレス
  - 実行に対してインスタンスを用意する必要無し
  - また、オートスケールにも対応し、可用性も高い

- 各種AWSサービスに対応
  - 実行ロールはIAM権限を付与する必要がある

### 11-2. Kinesis

![](2023-03-15-14-06-17.png)

### 11-3. ビッグデータ・IoT

![](2023-03-15-14-08-15.png)

### 11-4. 移行系サービス

![](2023-03-15-14-08-54.png)

### 11-5. コスト管理サービス

![](2023-03-15-14-11-00.png)
