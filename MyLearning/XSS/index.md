...
問題なく反映されています。
とある「User2」は、投稿する内容に `<s>test</s>` と入力して投稿しました。結果が以下です。
...
DOMとは、抽出した要素1つ1つのことを指しています。htmlをrootとして下位要素に枝分かれし、逆さまにした木(Tree)のように見える事からDOMツリーと呼ばれています。
...
先ほど文字の色を変更しましたが、これはHTMLファイルを書き換えたわけではありません。HTMLファイルを書き換える事なく、DOM( `p` タグの `id=tree` )にアクセスして、JavaScriptを使用して文字の色を変更しました。つまり、**DOMを使用するとサーバを介さずにHTMLファイルを操作することが可能ということ**です。
...
もう少し分かりやすく説明します。
例えば、次のような攻撃者がDOM操作を行なって用意したボタンがあるとします。
