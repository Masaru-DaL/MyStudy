...
リダイレクトはWebページが開かれた際に強制的にページ遷移を行うというものです。今回は[Envader](https://envader.plus)にリダイレクトが行われているため問題ありません。攻撃者によってリダイレクトが用意された場合、アクセスしたユーザに対して悪意のあるスクリプトが実行されるなど、**ページ遷移した先が危険である可能性があります**。
...
反射型XSSは、まず攻撃されたユーザは何らかの形でスクリプトを含んだリクエストをサーバに送信します。リクエストに含まれたスクリプトは、リクエストを送ったユーザに返ってくる事から、反射型XSSと呼ばれています。反射型XSSを受けてからの流れを、
