...
XSSはスクリプトを悪用して行われる攻撃です。まずはスクリプトが通常に動作した場合の挙動について理解する必要があります。
前述しましたが、本記事で言うスクリプトとは、JavaScriptのことを指します。つまり、JavaScriptが正常に動作した場合を解説します。
...
![](2022-10-17-09-20-39.png)
上の画像のような現在時刻をポップアップで表示した後に、ブラウザに「Hello World!!」と表示されます。このように、JavaScriptはアクセスした時の現在時刻を表示させることや、ポップアップでWebサイトのUIに動きを付ける事が出来ます。前述したように、動的なWebサイトを作成することが出来るということです。
...
```shell:
├── bad_script.html # 悪意を持って作られたスクリプトが動作するページ(リダイレクト, クッキーの値)
└── normal_script.html # 通常のスクリプトの動作を行うページ(現在時刻, Hello World!!)
```
