...
**パッケージの使用**でWebサーバを建てられる。最低限の機能実装の場合(リクエスト＆レスポンス)、必要なのは標準ライブラリの"fmt", "net/http"のみ。
main関数内でハンドラ(httpリクエストを受ける関数)を宣言し、http. ListenAndServeでポートを指定する。これだけでリクエストを待ち受けるWebサーバが建てられる。
