...
* パイロットライト戦略
パイロットライトとは、ガス駆動ヒーターなどの設備が必要に応じて素早く起動できるように常に点灯している小さな炎のことです。パイロットライトのように、DRリージョンに停止したサーバを用意しておき、EC2などのIaaSを用いて別リージョンにシステムを構築するといった手法を意味します。
基本的にはAmazon Route 53というサービスで接続先設定の変更によって切り替えを行います。**この戦略では、コストを抑えつつリージョンレベルの可用性を持たせることが可能になります**。

* ウォームスタンバイ戦略
パイロットライト戦略に似ていますが、DRリージョンに用意しておくのは常時起動したサーバです。待機系なので、同構成ですがリクエストを処理できる最低限のシステム(スペックを下げたシステム)を用意します。
