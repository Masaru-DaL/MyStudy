...
IaaSの代表的なサービスであるAmazon EC2(**A**mazon **E**lastic **C**ompute **C**loud)は、仮想サーバを構築できるサービスです。IaaSのサービス形態上、EC2を利用する場合はユーザ自身でEC2の可用性を高める対策を行う必要がありその方法を説明していきます。
...
ELBには大きく分けて4種のサービスがあり、その特徴などが以下の通りです。ロードバランサーを導入する際にはそれぞれのサービスの特徴に合わせて導入を検討しましょう。また、AWSは、よくある質問でアプリケーションに最適なロードバランサーを提案しています。(参照: Elastic Load Balancingのよくある質問)
