...
ユーザが利用するサービスによって高可用性を確保する難易度は違います。クラウドサービスは、開発者の利用や企業のシステムをオンプレミスからクラウドへ移行するなどで利用されることは少なくありません。AWSを利用する上で高可用性を確保するために知っておかなくていけないことを説明していきます。
...
IaaSの代表的なサービスであるAmazon EC2(**A**mazon **E**lastic **C**ompute **C**loud)は、仮想サーバを構築できるサービスです。IaaSのサービス形態上、EC2を利用する場合はユーザ自身でEC2の可用性を高める対策を行う必要があります。

<<<<<<< HEAD
Webサーバはアクセス状況の変化の影響を受けやすく、複雑なプログラムが動作する事からトラブルが発生しやすいです。また、インターネットに直接接続されているため、攻撃のターゲットとなりやすいといった性質もあります。

1つ目の方法は、複数のアベイラビリティゾーンによる冗長構成をとることで、**Multi-AZ構成**と呼ばれます。Multi-AZ構成は、同一リージョン内の複数AZにWebサーバを設置したシステう構成のことです。Webサーバを設置するAZを増やす毎に可用性は高くできますが、その分コストが高くなったり、レイテンシ(通信遅延時間)が増えやすくなるというデメリットもあります。
=======
...
>>>>>>> 894badf791e24bc6338533bb74e6460f57f4fb3f

もう1つの方法は、**ロードバランサー**を使用することです。AWSのロードバランサーはELB(**E**lastic **L**oad **B**alancing)と呼ばれており、大きく分けて4種のサービスがあります。
