...
例えば、AWSで提供しているデータベースサービスにARDS(**A**mazon **R**elational **D**atabase **S**ervice)があります。ARDSのバックアップの設定は自身で設定することになります。AWSのバックアップ料金は、バックアップの回数、復元するバックアップデータの量などによって変わってきます。AWSが提供する各サービスごとに、どのように定期的なバックアップを取ることができるのかを把握することが大事です。

### AWSマネージドサービス

AWSは**マネージドサービス**を提供しています。マネージド(Managed: 管理)が意味する通り、AWS側によってサービスを運用・管理してくれるサービスです。
