...
上述した公式ドキュメントの内容を、実際にPythonでクラスを実装しながら説明していきます。
...
オブジェクトは**さまざまな実体の総称**を指し、インスタンスは**クラスを元に作成したオブジェクトの実体のこと**を指します。クラスには、クラスオブジェクトと、インスタンスオブジェクトが存在します。

`test1 = TestClass()`で`test1`というインスタンスが作成されました。インスタンスの作成元の`TestClass`には**クラスオブジェクト**が代入されていて、`test1`には**インスタンスオブジェクト**が代入されています。
