...
「実際にクラスを使ってみよう」で行った例を再度見てみましょう。関数だけで実装した場合、`man1`や`female1`は「**何にでもなれます**」。クラス定義したものを③で実体化させ、④でクラスの関数を呼び出しています。クラス内の関数カプセル化の狙いは「**データを保護する**」ことにあります。カプセル化を行うと、クラスの中の関数をいじらない限りデータを操作することができなくなります。そうすることで外から不用意にデータを書き換えられたりする事がなくなり、データのバグを防ぐことができます。
