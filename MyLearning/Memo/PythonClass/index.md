...
- [Python クラス入門](#python-クラス入門-1)
  ...
  - [クラスを使用するメリット・使い所](#クラスを使用するメリット使い所)
  - [Python公式ドキュメントを確認する](#python公式ドキュメントを確認する)
  - [クラスの定義方法](#クラスの定義方法)
...
これがクラスを使用した例です。クラス内の関数に`__init__`や`self`という見慣れないものがあると思いますが、今は気にしないでください。後で説明していきます。
関数の時より長くなっていて逆に分かりづらいと思うかもしれません。コードが関数だけの時より長くなったのは、今回は**カプセル化**という処理を施したためです。つまり、クラスはただ使えば良いというわけではなく、クラスを使うと何が出来るのかを理解することが重要です。
何となく雰囲気が掴めた所で、今回のカプセル化も合わせて、「クラスを使用するメリット・使い所」について説明していきます。
...
「実際にクラスを使ってみよう」で行った「カプセル化」を再度見てみましょう。②で変数や関数はクラス内に全て記述しています。そうすると、変数に値を入れたり、関数を呼び出す際（④の処理のこと）は、クラス定義したものを③で実体化させた`man1`, `female1`を必ず通さないといけなくなります。

こうすると何が良いのかと言うと、外から不用意にデータを書き換えられたりする事がなくなり、データのバグを防ぐことができるようになります。つまり、カプセル化の処理を行うと、「**データを保護することができる**」ということです。
...
例えば人間クラスというものに人間として共通する処理を定義し、その人間クラスの関数を引き継いで男性クラス、女性クラスを作成する事が可能です。その男性クラスは「人間の共通部分 + 男性として共通する処理」、女性クラスは「人間の共通部分 + 女性として共通する処理」を定義します。そして、男性クラス・女性クラスから実体としての個々の男性・女性を作成する、という事ができます。こういった大元のクラスから共通した機能を引き継いで新しいクラスを作成することを**継承**と呼びます。
...
クラスを使うと、例えば鳴くという処理`cry()`を実行した時、呼び出す実体によって鳴き声を変えることができます。犬なら「わんわん」と鳴き、牛なら「もーもー」、豚なら「ぶーぶー」と1つの`cry()`を実体独自に実装することが可能です。呼び出した側は同じ`cry`でも、命令を受け取った側でそれぞれ異なる動作をすることを**ポリモーフィズム**と言います。

**ポリモーフィズム**

今回紹介した「継承」、「カプセル化」、「ポリモーフィズム」はクラスを使用することで受けられる大きなメリットであり、重要な概念です。これらの概念を使いこなすためにも、まずは「クラスとは何か」をしっかり理解する必要があります。
...
引用した文を数行と短いですが、これらを理解するには書かれている以上のことを理解する必要があります。今回は実際にPythonでクラスの定義やクラスに関するコーディングを行いながら説明していきます。
...
上記のように、`class`に続けてクラス名、クラスの実装内容を記述するだけでクラス定義ができます。Pythonのクラス名は**CapWords 方式**（**アッパーキャメルケース**）で記述するという命名規則がありますので、**クラス名は単語の先頭を大文字にして名付ける**点に注意しましょう。([Python コーディング規約 PEP 8: クラスの名前](https://pep8-ja.readthedocs.io/ja/latest/#section-26))
...
上記のように、クラスはあくまで手段を提供するだけで、**クラスを定義しただけではまだ実際に何も作られていない**ということです。インスタンスを作成は以下のように行います。
