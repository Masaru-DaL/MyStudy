...
実際にクラスを使ってみて何となく雰囲気は掴めたと思います。ここまでの例ではクラスを使用することによるメリットがイメージできないと思うので、「クラスを使用するメリット・使い所」について説明していきます。

## クラスを使用するメリット・使い所

- 共通処理をまとめたい時

関数でいいじゃんと思うかもしれませんが、クラスを使用するともっと大きな枠で処理を共通化する事ができます。例えば人間クラスというものに人間として共通する部分を定義し、その人間クラスを引き継いで更に男性クラス、女性クラスを作成する事が可能です。男性クラスは男性として共通する処理、女性クラスは女性として共通する処理を定義します。そして、実体としての男性、女性を作成する、という事ができます。この処理の流れのことを**継承**と呼びます。
...
最初の例で行ったカプセル化が2つ目のメリットになります。カプセル化の狙いは「**データを保護する**」ことにあります。カプセル化を行うと、クラスの中の関数をいじらない限りデータを操作することができなくなります。そうすることで外から不用意にデータを書き換えられたりする事がなくなり、データのバグを防ぐことができます。

**カプセル化の図**

- 同じ処理でも実体によって使い分けられる

クラスを使うと、例えば鳴くという処理`cry()`を実行した時、呼び出す実体によって鳴き声を変えることができます。犬なら「わん」と鳴き、猫なら「にゃー」、豚なら「ぶーぶー」と1つの`cry()`を実体独自に実装することが可能です。呼び出した側は同じ`cry`でも、命令を受け取った側でそれぞれ異なる動作をすることを**ポリモーフィズム**と言います。

**ポリモーフィズム**

今回紹介した「継承」、「カプセル化」、「ポリモーフィズム」はクラスを使用することで受けられる大きなメリットであり、重要な概念です。これらの概念をしっかり理解するためにも、まずは「クラスとは何か」をしっかり理解する必要があります。
