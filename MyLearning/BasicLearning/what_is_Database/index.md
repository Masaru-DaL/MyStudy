...
* 相違点
  ...
  + データにアクセスできる人
  + データを保存できる量
  + スプレッドシートはセルでデータを持つのに対し、データベースは列自体がデータを持つ
...
> - データ・レジデンシ、データ主権、またはレイテンシの要件を保証します。組織によっては、オンプレミスでの実行に適したユース・ケースがあります。そのような場合は、データベースを実行するために事前に構成および最適化されたエンジニアド・システムが理想的です。Wikibonの最近の分析 (PDF)によると、Oracle Exadataを使用することで、可用性とパフォーマンスの向上、および最大40％のコスト低減を実現できることがわかっています。

- データベース管理者には抱える問題が様々あった。
...
- 自律テクノロジーによりデータベース管理者の抱える多くの課題を解決できる
