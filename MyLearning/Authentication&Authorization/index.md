...
ログイン機能がないアプリケーションの場合、認証がなくてもユーザ全員の権限を一定にするだけで認証がなくても機能します。

しかし、用途によっては同じアプリケーションでも権限を調整したいということはありませんか？1つ例に挙げるとすれば、社内で同じアプリケーションを使用する場合です。
社長は全ての情報を見れる必要がありますが、社員は機密情報までを見れる必要はありません。また部署ごとに開示したり見れる情報もコントロールしたいと思うはずです。

### 認証と認可は組み合わせてこそ真価を発揮する

認証と認可は単体だけで機能するとすれば「認可」だけです。
しかし、認証と組み合わせることでアプリケーションに柔軟性を持たせ、様々なユーザに対応させることができます。

認可が実装されたアプリケーションは、不特定多数に対して使われるアプリケーションではなく、ログイン機能が備わり、1人のユーザとしてアカウントを持たせるアプリケーションで必要とされます。
