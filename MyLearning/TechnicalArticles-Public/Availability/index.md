...
<<<<<<< HEAD
EC2の可用性を高めるための1つ目の方法は、複数のAZ(アベイラビリティゾーン)による冗長構成をとることです。これは**Multi-AZ構成**と呼ばれます。AZを増やせばその分可用性は高くできますが、その分コストが高くなったり、レイテンシ(通信遅延時間)が増えやすくなるというデメリットもあります。
=======
* Multi-AZ構成
...
>>>>>>> 981e0ba82d586322edb7c611b7b801f8bbff428f
...
障害復旧は**ディザスタリカバリ**(DR)と呼ばれ、障害発生時に退避するリージョンをDRリージョンと呼ぶことがあります。DR戦略の基本となる複数リージョン・複数アベイラビリティーゾーンで構成すること以外にもいくつかDR戦略が存在します。AWSを利用してRPO・RTOを最適値にするためにも、障害復旧のための戦略を把握することが必要です。

* パイロットライト戦略
パイロットライトとは、ガス駆動ヒーターなどの設備が必要に応じて素早く起動できるように常に点灯している小さな炎のことです。パイロットライトのように、DRリージョンに**停止したサーバを用意**しておき、EC2などのIaaSを用いて別リージョンにシステムを構築するといった手法を意味します。
基本的にはAmazon Route 53というサービスで接続先設定の変更によって切り替えを行います。**この戦略では、コストを抑えつつリージョンレベルの可用性を持たせることが可能になります**。

* ウォームスタンバイ戦略
パイロットライト戦略に似ていますが、DRリージョンに**用意しておくのは常時起動したサーバ**です。待機系なので、同構成ですがリクエストを処理できる最低限のシステム(スペックを下げたシステム)を用意します。
障害発生時は素早くDRリージョンに切り替え、必要に応じてスケールアップ(スペックを上げる・運用系のスペックに戻す)します。

* マルチサイトアクティブ/アクティブ
ホットスタンバイとも呼ばれます。**運用系・待機系の両方ともスペックも含め全く同じ構成をDRリージョンに用意**しておきます。ビジネスモデルによってはコストがかかっても一切のダウンを許さないシステムもあるでしょう。その場合はこの構成を取ることが可能です。

* バックアップと復元
**定期的にバックアップを取り、リージョンが回復してから可能な限り素早くシステムを回復できるように備える**のも立派なDR戦略です。
例えばEC2上にデータベースを構築した場合、バックアップの設定は自身で設定することになります。AWSのバックアップ料金は、バックアップの回数、復元するバックアップデータの量などによって変わってきます。AWSが提供する各サービスごとに、どのように定期的なバックアップを取ることができるのかを把握することが大切です。

### AWSマネージドサービスを利用する

AWSは**マネージドサービス**を提供しています。マネージド(Managed: 管理)が意味する通り、AWS側によってサービスを運用・管理してくれるサービスです。一番分かりやすいのがSaaSモデルです。AWSで提供するSaaSのMicrosoft Office 365の場合、ユーザはただソフトウェアを使用するだけで、利用者で何かを管理する必要がありません。

前述したように、EC2上にデータベースを構築した場合、OSから上のレイヤは利用者側で管理を行う必要があります。具体的な管理範囲としては、OSの稼働監視・データの同期・バックアップ・障害時の切り替えといった設計・管理です。

しかし、データベースのマネージドサービスのARDS(**A**mazon **R**elational **D**atabase **S**ervice)を利用した場合、OSとある程度のデータベースのエンジンの設定はAWS側で行ってくれます。利用者としては運用で考慮すべき項目を少なくする事ができるというメリットがあります。こういったマネージドサービスを利用することで手軽に可用性を高めることができます。

## まとめ

今回は可用性について解説しました。

クラウドの導入は手軽に行える反面、提供されているサービス量が膨大なこともあり、サービスを適切に利用することが非常に困難です。しかし、可用性を高めるために行わなければいけない事さえ分かっていれば、後はそれが実現できるサービスを選ぶだけです。

可用性の正しい知識を身につけ、自身が携わるサービスの可用性を高めていきましょう。
