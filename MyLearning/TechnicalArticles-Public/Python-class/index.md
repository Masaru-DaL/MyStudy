...
上記がクラスを使用した例です。クラス内の関数に`__init__`や`self`という見慣れないものがあると思いますが、後で説明しますので、今は気にしなくて大丈夫です。
...
クラスを使うと、大元の「鳴く」という処理は同じだけど、そこから派生した先では違う「鳴き声」を実装したい、という事ができます。こういった呼び出した側は同じ処理でも、命令を受け取った側でそれぞれ異なる動作をすることを**ポリモーフィズム**と言います。
